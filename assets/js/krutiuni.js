...
function krutiunicode() {
  ...
  while (r == 1) {
    ...
    if (o < text_size - max_text_size) {
      o += max_text_size;
      while (document.getElementById("krutitext").value.charAt(o) != " ") {
        o--;
      }
    } else {
      o = text_size;
  ...
  function replsym() {
    if (kruti_text != "")
      for (
        input_symbol_idx = 0;
        input_symbol_idx < kruti_array_length;
        input_symbol_idx++
      ) {
        idx = 0;
...
function u_word() {
  ...
  var sourceHTML =
    header + document.getElementById("unicodetext").value + footer;
  ...
  fileDownload.download = "egconvert.doc";
  fileDownload.click();
